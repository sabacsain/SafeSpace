...
const CommunityCard = props => {
  return (
    <div className='flex justify-center items-center w-full h-[65vh] px-20'>
      <div className="flex gap-20 h-fit">
      { props.title === "7 Cups" ? (
      <>
        <div className=''>
          <h1 className={`text-4xl font-bold text-secondary-200 text-left mb-4`}>{props.title}</h1>
          <p className='mb-5 text-black text-justify'>{props.children}</p>
          <Button outsideLink={props.link}>Learn More</Button>
        </div>
...
const Communities = () => {
  return (
    <>
      ...
      <div class='pt-10'></div>
      <HeaderText header='We are here to Support You'
                  description='If you need someone to talk with, they are here for you.'></HeaderText>
      <div className='flex flex-col justify-center items-center'>
        <Fade direction='right' duration={1750}>
        <CommunityCard image={nami} 
                       itle="NAMI (National Alliance on Mental Illness)"  
